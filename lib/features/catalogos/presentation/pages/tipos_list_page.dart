...
class _TiposListView extends StatelessWidget {
  ...
  Widget _buildCounter(BuildContext context, TiposState state) {
    int activos = 0;
    int inactivos = 0;

    if (state is TiposLoaded) {
      activos = state.tipos.where((t) => t.activo).length;
      inactivos = state.tipos.where((t) => !t.activo).length;
    }
    ...
    return Container(
      ...
      child: Text(
        '$activos tipos activos / $inactivos inactivos',
        style: TextStyle(
          fontSize: 14,
