-- Migration: Usar Edge Function para envío de emails
-- Fecha: 2025-10-04
-- Razón: pg_net no puede conectarse directamente a Inbucket SMTP, usar Edge Function como proxy

BEGIN;

-- ============================================
-- Función actualizada para enviar emails via Edge Function
-- ============================================
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #F9FAFB;">
    ...
    -- URL de la Edge Function (dentro de la red Docker de Supabase)
    -- Kong (API Gateway) está en http://kong:8000
    v_function_url := 'http://kong:8000/functions/v1/send-email';

    -- Enviar solicitud HTTP POST a la Edge Function
    SELECT INTO v_response_id net.http_post(
        ...
        body := jsonb_build_object(
            ...
            'fromName', 'Sistema de Gestión de Medias'
        )
    );

    -- Log del envío (para debug)
    RAISE NOTICE 'Email de confirmación enviado a % via Edge Function (response_id: %)', p_to_email, v_response_id;
...
-- Comentario
COMMENT ON FUNCTION send_confirmation_email IS 'HU-001: Envía email vía Edge Function send-email que usa SMTP de Inbucket';
