...
-- ============================================
-- SECCIÓN 7: FUNCIONES DE PRODUCTOS MAESTROS (HU-006)
-- ============================================

-- HU-006: validar_combinacion_comercial - Valida combinaciones tipo + sistema talla
...
DECLARE
    ...
    v_error_hint TEXT;
BEGIN
    -- Obtener información del tipo y sistema de tallas
    ...
    WHERE st.id = p_sistema_talla_id;

    -- Validar existencia
    IF v_tipo_nombre IS NULL THEN
        v_error_hint := 'tipo_not_found';
        RAISE EXCEPTION 'Tipo de media no encontrado';
    END IF;
    ...
    -- Validaciones de combinaciones comerciales (RN-040)
    -- Tipo "Futbol" + Sistema "ÚNICA"
    IF LOWER(v_tipo_nombre) LIKE '%futbol%' AND v_sistema_tipo = 'UNICA' THEN
        v_warnings := array_append(v_warnings, 'Las medias de fútbol normalmente usan tallas numéricas (35-44)');
    END IF;

    -- Tipo "Futbol" + Sistema "LETRA"
    IF LOWER(v_tipo_nombre) LIKE '%futbol%' AND v_sistema_tipo = 'LETRA' THEN
        v_warnings := array_append(v_warnings, 'Las medias de fútbol normalmente usan tallas numéricas, no S/M/L');
    END IF;

    -- Tipo "Invisible" + Sistema "LETRA"
    IF LOWER(v_tipo_nombre) LIKE '%invisible%' AND v_sistema_tipo = 'LETRA' THEN
        v_warnings := array_append(v_warnings, 'Las medias invisibles suelen ser talla única o numérica');
    END IF;

    -- Tipo "Invisible" + Sistema "NÚMERO"
    IF LOWER(v_tipo_nombre) LIKE '%invisible%' AND v_sistema_tipo = 'NUMERO' THEN
        v_warnings := array_append(v_warnings, 'Las medias invisibles suelen ser talla única');
    END IF;

    -- Tipo "Tobillera" + Sistema "ÚNICA" (advertencia menos crítica)
    IF LOWER(v_tipo_nombre) LIKE '%tobillera%' AND v_sistema_tipo = 'UNICA' THEN
        v_warnings := array_append(v_warnings, 'Verifique que talla única sea apropiada para este tipo de media');
    END IF;
...
COMMENT ON FUNCTION validar_combinacion_comercial IS 'E002-HU-006: Valida combinaciones comerciales tipo + sistema talla (advertencias no bloqueantes) - RN-040';

-- HU-006: crear_producto_maestro - Crea producto maestro con validaciones
...
DECLARE
    ...
    v_combinacion_existe BOOLEAN;
    v_combinacion_inactiva BOOLEAN;
    v_producto_inactivo_id UUID;
    v_warnings TEXT[] := ARRAY[]::TEXT[];
    v_validacion_comercial JSON;
    v_error_hint TEXT;
BEGIN
    -- Validación RN-039: Longitud descripción
    IF p_descripcion IS NOT NULL AND LENGTH(p_descripcion) > 200 THEN
        v_error_hint := 'invalid_description_length';
        RAISE EXCEPTION 'La descripción no puede exceder 200 caracteres';
    END IF;

    -- Validación RN-038: Verificar que todos los catálogos estén activos
    SELECT activo, nombre INTO v_marca_activo, v_marca_nombre
    FROM marcas WHERE id = p_marca_id;

    SELECT activo, nombre INTO v_material_activo, v_material_nombre
    FROM materiales WHERE id = p_material_id;

    SELECT activo, nombre INTO v_tipo_activo, v_tipo_nombre
    FROM tipos WHERE id = p_tipo_id;

    SELECT activo, nombre INTO v_sistema_activo, v_sistema_nombre
    FROM sistemas_talla WHERE id = p_sistema_talla_id;

    -- Verificar existencia de catálogos
    IF v_marca_nombre IS NULL THEN
        v_error_hint := 'marca_not_found';
        RAISE EXCEPTION 'La marca especificada no existe';
    END IF;

    IF v_material_nombre IS NULL THEN
        v_error_hint := 'material_not_found';
        RAISE EXCEPTION 'El material especificado no existe';
    END IF;

    IF v_tipo_nombre IS NULL THEN
        v_error_hint := 'tipo_not_found';
        RAISE EXCEPTION 'El tipo especificado no existe';
    END IF;

    IF v_sistema_nombre IS NULL THEN
        v_error_hint := 'sistema_not_found';
        RAISE EXCEPTION 'El sistema de tallas especificado no existe';
    END IF;

    -- Verificar que estén activos
    IF NOT v_marca_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'La marca "%" está inactiva', v_marca_nombre;
    END IF;

    IF NOT v_material_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'El material "%" está inactivo', v_material_nombre;
    END IF;

    IF NOT v_tipo_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'El tipo "%" está inactivo', v_tipo_nombre;
    END IF;

    IF NOT v_sistema_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'El sistema de tallas "%" está inactivo', v_sistema_nombre;
    END IF;

    -- Validación RN-037: Verificar unicidad de combinación (incluso inactivos)
    SELECT
        EXISTS(SELECT 1 FROM productos_maestros
        ...
        EXISTS(SELECT 1 FROM productos_maestros
               ...
               AND sistema_talla_id = p_sistema_talla_id
               AND activo = false)
    INTO v_combinacion_existe, v_combinacion_inactiva;

    -- CA-016: Si existe combinación inactiva, ofrecer reactivar
    IF v_combinacion_existe THEN
        IF v_combinacion_inactiva THEN
            SELECT id INTO v_producto_inactivo_id
            ...
            RAISE EXCEPTION 'Ya existe un producto inactivo con esta combinación. ID: %', v_producto_inactivo_id;
        ELSE
    ...
    -- Validación RN-040: Advertencias de combinaciones comerciales (no bloqueante)
    SELECT validar_combinacion_comercial(p_tipo_id, p_sistema_talla_id) INTO v_validacion_comercial;

    IF (v_validacion_comercial->'data'->>'has_warnings')::boolean THEN
        v_warnings := ARRAY(
            SELECT json_array_elements_text(v_validacion_comercial->'data'->'warnings')
        );
    END IF;
    ...
    RETURN json_build_object(
        ...
        'data', json_build_object(
            ...
            'tipo_id', p_tipo_id,
            'sistema_talla_id', p_sistema_talla_id,
            'descripcion', p_descripcion,
            'nombre_completo', v_marca_nombre || ' - ' || v_tipo_nombre || ' - ' || v_material_nombre || ' - ' || v_sistema_nombre,
            'warnings', v_warnings,
...
COMMENT ON FUNCTION crear_producto_maestro IS 'E002-HU-006: Crea producto maestro con validaciones de unicidad y catálogos activos - RN-037, RN-038, RN-039, RN-040';

-- HU-006: listar_productos_maestros - Lista productos maestros con filtros
...
BEGIN
    SELECT json_agg(
        json_build_object(
            ...
            'marca_codigo', m.codigo,
            'marca_activo', m.activo,
            'material_id', pm.material_id,
            'material_nombre', mat.nombre,
            'material_codigo', mat.codigo,
            'material_activo', mat.activo,
            'tipo_id', pm.tipo_id,
            'tipo_nombre', t.nombre,
            'tipo_codigo', t.codigo,
            'tipo_activo', t.activo,
            'sistema_talla_id', pm.sistema_talla_id,
            'sistema_talla_nombre', st.nombre,
            'sistema_talla_tipo', st.tipo_sistema,
            'sistema_talla_activo', st.activo,
            'descripcion', pm.descripcion,
            'activo', pm.activo,
            'created_at', pm.created_at,
            'updated_at', pm.updated_at,
            'nombre_completo', m.nombre || ' - ' || t.nombre || ' - ' || mat.nombre || ' - ' || st.nombre,
            -- RN-043: Contador de artículos derivados (0 por ahora, tabla articulos en HU-007)
            'articulos_activos', 0,
            'articulos_totales', 0,
            -- RN-038: Badge si contiene catálogos inactivos
            'tiene_catalogos_inactivos', (NOT m.activo OR NOT mat.activo OR NOT t.activo OR NOT st.activo)
        )
    ...
    WHERE
        (p_marca_id IS NULL OR pm.marca_id = p_marca_id)
        ...
        AND (
            ...
            OR pm.descripcion ILIKE '%' || p_search_text || '%'
            OR m.nombre ILIKE '%' || p_search_text || '%'
            OR mat.nombre ILIKE '%' || p_search_text || '%'
            OR t.nombre ILIKE '%' || p_search_text || '%'
            OR st.nombre ILIKE '%' || p_search_text || '%'
        );
...
COMMENT ON FUNCTION listar_productos_maestros IS 'E002-HU-006: Lista productos maestros con filtros y detección de catálogos inactivos - RN-038, RN-043';

-- HU-006: editar_producto_maestro - Edita producto maestro con restricciones
...
BEGIN
    ...
    SELECT activo INTO v_producto_activo
    FROM productos_maestros
    ...
    -- RN-043: Contar artículos derivados (0 por ahora, tabla articulos en HU-007)
    -- En el futuro: SELECT COUNT(*) INTO v_articulos_totales FROM articulos WHERE producto_maestro_id = p_producto_id;
    v_articulos_totales := 0;

    -- RN-044: Si tiene artículos derivados, solo permitir editar descripción
    ...
    ELSE
        -- Sin artículos derivados: permitir editar todos los campos

        -- Si se cambian catálogos, validar que existan y estén activos
        IF p_marca_id IS NOT NULL THEN
            SELECT activo, nombre INTO v_producto_activo, v_marca_nombre
            FROM marcas WHERE id = p_marca_id;
        ...
        IF p_material_id IS NOT NULL THEN
            SELECT activo, nombre INTO v_producto_activo, v_material_nombre
            FROM materiales WHERE id = p_material_id;
        ...
        IF p_tipo_id IS NOT NULL THEN
            SELECT activo, nombre INTO v_producto_activo, v_tipo_nombre
            FROM tipos WHERE id = p_tipo_id;
        ...
        IF p_sistema_talla_id IS NOT NULL THEN
            SELECT activo, nombre INTO v_producto_activo, v_sistema_nombre
            FROM sistemas_talla WHERE id = p_sistema_talla_id;
...
COMMENT ON FUNCTION editar_producto_maestro IS 'E002-HU-006: Edita producto maestro con restricciones si tiene artículos derivados - RN-044';

-- HU-006: eliminar_producto_maestro - Elimina permanentemente si no tiene artículos
...
BEGIN
    -- Verificar que el producto existe
    SELECT EXISTS(SELECT 1 FROM productos_maestros WHERE id = p_producto_id)
    ...
    -- RN-043: Si tiene artículos, rechazar eliminación
    IF v_articulos_totales > 0 THEN
        v_error_hint := 'has_derived_articles';
        RAISE EXCEPTION 'No se puede eliminar. Este producto tiene % artículos. Solo puede desactivarlo', v_articulos_totales;
    END IF;
    ...
    RETURN json_build_object(
        ...
        'data', json_build_object(
            'id', p_producto_id,
            'message', 'Producto maestro eliminado permanentemente'
        )
    );
...
COMMENT ON FUNCTION eliminar_producto_maestro IS 'E002-HU-006: Elimina permanentemente producto maestro solo si no tiene artículos derivados - RN-043';

-- HU-006: desactivar_producto_maestro - Desactiva producto maestro y opcionalmente artículos
...
DECLARE
    v_producto_existe BOOLEAN;
    v_articulos_activos INTEGER := 0;
    v_articulos_desactivados INTEGER := 0;
    v_error_hint TEXT;
BEGIN
    -- Verificar que el producto existe
    SELECT EXISTS(SELECT 1 FROM productos_maestros WHERE id = p_producto_id)
    ...
    -- RN-042: Desactivar producto maestro
    UPDATE productos_maestros
    SET activo = false, updated_at = NOW()
    WHERE id = p_producto_id;

    -- RN-042: Contar artículos activos (0 por ahora, tabla articulos en HU-007)
    -- En el futuro: SELECT COUNT(*) INTO v_articulos_activos FROM articulos WHERE producto_maestro_id = p_producto_id AND activo = true;
    v_articulos_activos := 0;

    -- RN-042: Si se solicita, desactivar artículos en cascada
    IF p_desactivar_articulos AND v_articulos_activos > 0 THEN
        -- En el futuro: UPDATE articulos SET activo = false, updated_at = NOW() WHERE producto_maestro_id = p_producto_id AND activo = true;
        v_articulos_desactivados := v_articulos_activos;
    END IF;
    ...
    RETURN json_build_object(
        ...
        'data', json_build_object(
            'id', p_producto_id,
            'articulos_activos_afectados', v_articulos_desactivados,
            'message', 'Producto maestro desactivado exitosamente'
        )
    );
...
COMMENT ON FUNCTION desactivar_producto_maestro IS 'E002-HU-006: Desactiva producto maestro y opcionalmente artículos derivados en cascada - RN-042';

-- HU-006: reactivar_producto_maestro - Reactiva producto maestro si catálogos están activos
...
BEGIN
    ...
    -- Verificar que todos los catálogos relacionados estén activos
    SELECT m.activo, m.nombre, mat.activo, mat.nombre, t.activo, t.nombre, st.activo, st.nombre
    ...
    -- Rechazar reactivación si algún catálogo está inactivo
    IF NOT v_marca_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'No se puede reactivar. La marca "%" está inactiva', v_marca_nombre;
    END IF;

    IF NOT v_material_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'No se puede reactivar. El material "%" está inactivo', v_material_nombre;
    END IF;

    IF NOT v_tipo_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'No se puede reactivar. El tipo "%" está inactivo', v_tipo_nombre;
    END IF;

    IF NOT v_sistema_activo THEN
        v_error_hint := 'inactive_catalog';
        RAISE EXCEPTION 'No se puede reactivar. El sistema de tallas "%" está inactivo', v_sistema_nombre;
    END IF;
...
COMMENT ON FUNCTION reactivar_producto_maestro IS 'E002-HU-006: Reactiva producto maestro solo si todos los catálogos relacionados están activos - RN-038';
